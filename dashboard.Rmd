...
So ist hier das Geschlecht die unabhängige und das Überleben die abhängige Variable. Tatsächlich sieht man, dass die meisten Männer ums Leben gekommen sind, während die meisten Frauen überlebt haben. Doch man sieht gleichzeitig, dass der männliche Segment per se größer ist als der weibliche - liegt also eine solche Verteilung tatsächlich am Geschlecht und nicht an der Tatsache, dass es mehr Männer als Frauen an Bord gab? Überprüfen wir das mal.

```{r init-var, echo=FALSE}
...
maenner_gestorben <- nrow(titanic_dt[(titanic_dt$Sex == "männlich") 
                                     & (titanic_dt$Survived == "nicht überlebt"),])
frauen_ueberlebt <- nrow(titanic_dt[(titanic_dt$Sex == "weiblich") 
                                     & (titanic_dt$Survived == "überlebt"),])
...
# Führe den Code aus. Alle absoluten Variablen wurden schon definiert.

anteil_maenner <- maenner / n_total # Gesamtanteil der Männer
anteil_frauen <- frauen / n_total # Gesamtanteil der Frauen

anteil_maenner_ueberlebt <- maenner_ueberlebt / maenner # Anteil Überlebender an Männern
anteil_maenner_gestorben <- maenner_gestorben / maenner # Anteil Gestorbener an Männern
anteil_frauen_ueberlebt <- frauen_ueberlebt / frauen # Anteil Überlebender an Frauen
anteil_frauen_gestorben <- frauen_gestorben / frauen # Anteil Gestorbener an Frauen

print(paste0(anteil_maenner_ueberlebt * anteil_maenner * 100, 
             "% der Passagiere haben überlebt, wenn sie männlich waren. ", 
             anteil_maenner_gestorben * anteil_maenner * 100, 
             "% der Passagiere sind hingegen gestorben, wenn sie männlich waren. ",
             "Dabei sind ", anteil_frauen_ueberlebt * anteil_frauen * 100, 
             "% überlebt und ", anteil_frauen_gestorben * anteil_frauen * 100, 
             "% gestorben, wenn sie Frauen waren."))
```
